        ...
        for (Field field : fields) {
            ...
            if (annotation != null) {
                ...
                try {
                    ...
                    Object o = method.invoke(view, value);
                    field.setAccessible(true);
            ...
            for (Annotation annotation : annotations) {
                ...
                if (eventBase != null) {
                    ...
                    Object listener = Proxy.newProxyInstance(listenerType.getClassLoader(),
                            new Class[]{listenerType}, handler);
                    try {
                        Method typeMethod = annotationType.getMethod("value");
                        int[] viewIds = (int[]) typeMethod.invoke(annotation);
                        for (int viewId : viewIds) {
                            Method findViewById = view.getClass().getMethod("findViewById", int.class);
                            View view1 = (View) findViewById.invoke(view, viewId);

                            Method setter = view1.getClass().getMethod(listenerSetter, listenerType);
                            setter.invoke(view1, listener);
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
